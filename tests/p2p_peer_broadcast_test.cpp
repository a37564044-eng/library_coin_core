...
int main() {
    std::cout
        << "=== LARB P2P PEER BROADCAST TEST ===\n";

    constexpr std::uint32_t difficulty = 3;

    constexpr std::uint16_t port_b = 18451;
    constexpr std::uint16_t port_c = 18452;
    ...
    const std::string tip_c =
        node_c.blockchain()
            .at(node_c.chain_size() - 1)
            .hash();
    ...
    if (!node_a.is_chain_valid() ||
        !node_b.is_chain_valid() ||
        !node_c.is_chain_valid()) {

        std::cerr
            << "All node chain validation: FAIL\n";
        return 1;
    }
    ...
    std::cout
        << "=== ALL P2P PEER BROADCAST TESTS PASSED ===\n";
